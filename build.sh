#!/bin/bash
#
# Kernel build script for OnePlus Nord CE3 Lite 5G (larry) with KernelSU support
#
# Thanks to @StratoNeutro for base script
# Thanks to @NoCache-69 for Helping
...
CLEAN_BUILD=false
DEFCONFIG="vendor/larry-kryon_defconfig"
...
for arg in "$@"; do
    case $arg in
        --kryon)
            ...
            DEFCONFIG="vendor/larry-kryon_defconfig"
            echo "==> Using Kryon config"
            ;;
        --help|-h)
            echo "Usage: $0 [OPTIONS]"
            echo ""
            echo "Options:"
            echo "  --kryon         Perform a clean build, using Kryon defconfig (mrproper)"
...
export KBUILD_BUILD_USER="noescape"
...
# begin properties
properties() { '
kernel.string=KryonKernel for OnePlus Nord CE3 Lite 5G
do.devicecheck=1
do.modules=1
do.systemless=1
do.cleanup=1
do.cleanuponabort=0
device.name1=larry
device.name2=OnePlus Nord CE3 Lite 5G
device.name3=CPH2467
device.name4=OP5958L1
device.name5=CPH2465
...
echo "==============================================="
echo "Device: OnePlus Nord CE3 Lite 5G (larry)"
